...
20120730094100&userip=192.168.0.1&url=%s" % (self._host, urllib2.quote(url, safe=''))
        ...
        if response.getcode() == httplib.OK:
            ...
            for e in element.iterfind("SD"):
                ...
                daypopularity = e.find("REACH")
                if popularity is not None:
                    return popularity.get("TEXT"),daypopularity.get("RANK")
...
class SogouRank(RankProvider):
    """ Get the Alexa Traffic Rank for a URL
 
    """
    def __init__(self, host="rank.ie.sogou.com", proxy=None, timeout=30):
        """Keyword arguments:
        host -- toolbar host address: Default: joolbarqueries.google.com
        proxy -- address of proxy server (if required). Default: None
        timeout -- how long to wait for a response from the server.
        ...
        """
        super(SogouRank, self).__init__(host, proxy, timeout)
        self._opener.addheaders = [("User-agent", "Mozilla/4.0 (compatible; \
GoogleToolbar 2.0.111-big; Windows XP 5.1)")]
    def get_rank(self, url):
        """Get the page rank for the specified URL
        ...
        """
        url = 'link:'+url
        reffer = "http://www.sogou.com/web?ie=utf8&query=%s&_ast=1366337536&_asf=null&w=01029901&p=40040100&dp=1&sut=4206&sst0=1366337505283"% (urllib2.quote(url, safe=''))
        self._opener.addheaders = [("Referer", reffer)]
        query = "http://%s/sogourank.php?ur=%s" % (self._host, urllib2.quote(url, safe=''))
        response = self._opener.open(query, timeout=self._timeout)
        data = response.read()
        '''print data
        regex = re.compile(r"<span id='sr_span'>(.*?)</span>")  
            #element = xml.etree.ElementTree.fromstring(data)
            #alexa_data_sum_day  = ''
        sogou_rank = regex.findall(data)'''
        return data
       
            
 
if __name__ == "__main__":
    #url = "http://www.cairenhui.com"
    url = "http://www.xincai888.com"
...
"""
    a = AlexaTrafficRank()
    print(AlexaTrafficRank().get_rank(url))
    print(GooglePageRank().get_rank(url))
